...
const MobileNav = React.memo(() => {
	...
	return (
		<div className={`sm:bg--to-b bg-gray-800 py-6 sm:from-[#00000060] sm:to-transparent ${hasGrayBg ? "bg-gray-800" : ""}`}>
			<div className="mx-auto flex items-center justify-between px-4">
				<NavLink to="/">
					<img src="/logo.svg" className="w-10" alt="venture logo" />
				</NavLink>
			...
			{isOpen && (
				<div className="fixed left-0 top-0 flex h-screen w-full flex-col items-center justify-center space-y-4 bg-black bg-opacity-50 py-3 text-white backdrop-blur-xl">
					...
					<hr className="w-[80%] opacity-30" />
					<Link target="_blank" to="https://venture.canny.io/feature-requests" className="transition-color text-2xl font-semibold duration-150 hover:text-blue-400" onClick={toggleMenu}>
						Feedback
					</Link>
					{user ? (
						<>
						...
						<NavLink to="/sign" onClick={toggleMenu} className={`button bg--to-bl w-[80%] from-sky-500 to-indigo-500 text-center text-2xl font-bold ${isHidden ? "hidden" : ""}`}>
							Sign In
						</NavLink>
					)}
				</div>
